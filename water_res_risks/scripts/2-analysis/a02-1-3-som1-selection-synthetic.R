...
df = readr::read_rds(here("data/som_files/som1_performance_synthetic/som_performance_combined.rds"))
...
nrow(df_keep) # 426 kept out of 1020 28.02.2026
#418 06.03.2026
...
nrow(df_keepcomb) # 288 kept out of 920 28.02.2026
#282 06.03.2026
...
ggplot() +
  ...
  coord_cartesian(ylim=c(0, 1), xlim = c(7.5, 36.5), expand = 0, clip = "off") +
...
# library(clusterSim)
# 
# # 1. Read all SOM performance files
# perf_files <- list.files("som_performance_synthetic", pattern = "*.rds", full.names = TRUE)
# df <- perf_files |> lapply(read_rds) |> bind_rows()
# 
# # Optional: save full unfiltered summary
# write_rds(df, here("data/synthetic_som1_performance_combined.rds"))
# 
# # 2. Remove outliers
# df_keep <- tibble()  # to hold filtered rows
# 
# global_mad_db <- df |> group_by(som_size) |> summarise(mad = mad(db_x, na.rm = TRUE)) |> pull(mad) |> mean(na.rm = TRUE)
# global_mad_kl <- df |> group_by(som_size) |> summarise(mad = mad(k_l, na.rm = TRUE)) |> pull(mad) |> mean(na.rm = TRUE)
# 
# for (sz in unique(df$som_size)) {
#   iter_df <- df %>% filter(som_size == sz)
#   
#   local_mad_db <- mad(iter_df$db_x, na.rm = TRUE)
#   local_mad_kl <- mad(iter_df$k_l, na.rm = TRUE)
#   
#   range_db <- min(global_mad_db, local_mad_db)
#   range_kl <- min(global_mad_kl, local_mad_kl)
#   
#   iter_df <- iter_df %>%
#     filter(between(db_x, median(db_x, na.rm = TRUE) - range_db, median(db_x, na.rm = TRUE) + range_db)) %>%
#     filter(between(k_l, median(k_l, na.rm = TRUE) - range_kl, median(k_l, na.rm = TRUE) + range_kl))
#   
#   df_keep <- bind_rows(df_keep, iter_df)
# }
# 
# write_rds(df_keep, here("data/synthetic_som1_filtered_stage1.rds"))
# 
# # 3. COmbined performance metric nd filtering
# # Normalize individual vectors with min-max scaling
# df_keep <- df_keep %>%
#   mutate(
#     dbi_scaled = clusterSim::data.Normalization(matrix(db_x), type = "n1", normalization = "column")[,1],
#     kl_scaled  = clusterSim::data.Normalization(matrix(k_l), type = "n1", normalization = "column")[,1],
#     perf = clusterSim::data.Normalization(matrix(dbi_scaled + kl_scaled), type = "n1", normalization = "column")[,1]
#   )
# 
# 
# # Remove outliers of combined score
# df_keepcomb <- tibble()
# global_mad_perf <- df_keep %>% group_by(som_size) %>% summarise(mad = mad(perf, na.rm = TRUE)) %>% pull(mad) %>% mean(na.rm = TRUE)
# 
# for (sz in unique(df_keep$som_size)) {
#   iter_df <- df_keep %>% filter(som_size == sz)
#   local_mad_perf <- mad(iter_df$perf, na.rm = TRUE)
#   range_perf <- min(global_mad_perf, local_mad_perf)
#   
#   iter_df <- iter_df %>%
#     filter(between(perf, median(perf, na.rm = TRUE) - range_perf,
#                    median(perf, na.rm = TRUE) + range_perf))
#   
#   df_keepcomb <- bind_rows(df_keepcomb, iter_df)
# }
# 
# write_rds(df_keepcomb, here("data/synthetic_som1_filtered_stage2.rds"))
# 
# # SOM size selection
# 
# # Best-performing model
# best_row <- df_keepcomb %>% filter(perf == min(perf, na.rm = TRUE)) %>% slice(1)
# best_size <- best_row$som_size
# best_iter <- best_row$som_iter
# 
# cat("✅ Best SOM1 synthetic model: size =", best_size, " iteration =", best_iter, "\n")
# write_rds(best_size, here("data/best_synthetic_som_size.rds"))
# write_rds(best_row, here("data/best_synthetic_som1_row.rds"))
# 
# 
...
# )
# 
# write_rds(iter_index, here("data/00_full_SOM_iteration_sizing_index.rds"))
