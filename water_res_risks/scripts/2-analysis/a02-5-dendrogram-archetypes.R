...
write.csv(
  archetype_clusters,
  here::here("data/output/archetype_cluster_mapping.csv"),
  row.names = FALSE
)
...
terra::writeVector(
  watersheds,
  here::here("data/output/water_res_risks_archetypes_cluster_5.gpkg"),
  filetype = "GPKG",
  overwrite = TRUE
)

# ### Further analysis of archetypes
# library(readr)
# library(here)
# 
# # Load the archetype summary table
# df <- read_csv(here("data/output/archetype_24_summary.csv"))
# 
# # Select  the numeric metric columns (drop archetypeID)
# metrics_mat <- metrics %>%
#   dplyr::select(ends_with("_z")) %>%
#   as.data.frame()
# 
# # Run PCA on that matrix
...
# library(ggplot2)
# 
# #  Select metrics (numeric variables only)
# metrics_mat <- df %>%
#   dplyr::select(ends_with("_gr")) %>%
#   as.data.frame()
# 
# # Compute Euclidean distance between archetypes
# dist_mat <- dist(scale(metrics_mat))
# 
# # Run classical MDS (cmdscale)
