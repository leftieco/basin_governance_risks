...
iter_index <- read_rds("data/som_files/som1_derivation_data/00_synthetic_SOM_iteration_sizing_index.rds")
...
dir.create("som1_files_synthetic", showWarnings = FALSE)
dir.create("som1_performance_synthetic", showWarnings = FALSE)
...
# install.packages("doRNG") if you want strict reproducibility
# library(doRNG)
# results <- foreach(...) %dorng% { ... }
...
foreach(i = seq_len(nrow(iter_index)),
  ...
  # Deterministic per-task seed (good enough for most cases)
  set.seed(100000 + i)
